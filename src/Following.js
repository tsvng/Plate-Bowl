...
export default class Following extends React.Component{
  async componentDidMount(){
    ...
    async function editOtherFollowingList(){
      ...
      if(await API.graphql(graphqlOperation(getUser, {username: term})).then((evt) =>evt.data.getUser) != null)
      {
        ...
        await API.graphql(graphqlOperation(updateUser, {input:{username: term, following: otherUserFollowingListArray}}));
      }
      else
        console.log("user being removed from followers dne");
    }
  ...
  render(){
    return <div id='main' className = "follow">
                <h1>Follow Page</h1>
                <input type="text" id="searchInput" placeholder="Enter a user you'd like to follow, or type a user already in your list to unfollow."/> 
